...
const BROADCASTER_ID = '258e0f7f-cdd0-4ab8-89f2-82d97993f474'; // AkaneDoThis Twitch ID
...
export default function Schedule() {
  ...
  const fetchScheduleData = useCallback(async () => {
    ...
    try {
      ...
      const scheduleData = await getChannelSchedule(BROADCASTER_ID, token);
    ...
    if (schedule.length > 0) {
      ...
      return DEFAULT_SCHEDULE.map((item, index) => (
        <div className="card-3d-container" key={index}>
          <div className={`neo-card neo-card-${item.color} card-3d p-6 text-center`}>
